...
export function ExpenseList({ expenses, month, onDelete, categoryFilter = null }: Props) {
  const visible = categoryFilter
    ...
    : expenses;
  return (
    <section className="panel transactions" aria-labelledby="transactions-title">
      <h2 id="transactions-title">Transactions</h2>
      {visible.length === 0 ? (
        categoryFilter ? (
          <p className="empty-state">
            No {categoryFilter} expenses in {formatMonth(month)}.
          </p>
        ) : (
              ...
              {visible.map((expense) => (
                <tr key={expense.id}>
                  ...
                  <td className="description">
                    {expense.description || <span className="muted">No description</span>}
                  </td>
                  <td className="numeric">
                    {CURRENCY} {formatAmount(expense.amountMinor)}
                  </td>
                  <td>
                    <button
                      type="button"
