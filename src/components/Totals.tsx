...
export function Totals({ totals, count, filter, onFilter }: Props) {
  return (
    <section className="panel totals-panel" aria-label="Monthly spending">
      <div className="summary-heading">
        <div>
          <p className="summary-label">Total spent</p>
          <p className="total-amount" aria-label="Total spent">
            {CURRENCY} {formatAmount(totals.totalMinor)}
          </p>
      ...
      {filter && (
        <p className="filter-hint">
          ...
          <button type="button" className="text-button" onClick={() => onFilter(null)}>
            Clear filter
          </button>
        </p>
      )}
