...
export function BudgetCard({ budgetMinor, spentMinor, month, today, onSave, stipendDay, onStipendSave }: Props) {
  ...
  return <section className="panel budget-panel" aria-labelledby="budget-title">
    ...
    {stipend && <p className="budget-help">{stipend.daysUntil === 0 ? 'Stipend day is today' : `Stipend in ${stipend.daysUntil} days (${formatDate(stipend.date)})`}</p>}
    {budgetMinor !== null && !editing && <button type="button" className="text-button" onClick={() => setEditing(true)} aria-label="Edit budget settings">Edit</button>}
    {(editing || budgetMinor === null) && <form onSubmit={submit} noValidate aria-label="Set monthly budget">
      ...
      <input id="stipend-input" type="number" min="1" max="31" step="1" value={stipendInput}
        onChange={event => setStipendInput(event.target.value)} aria-invalid={Boolean(stipendError)} aria-describedby="stipend-error" />
      <p id="stipend-error" className="field-error" aria-live="polite">{stipendError}</p>
