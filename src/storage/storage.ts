...
export const RECOVERY_NOTICE = 'Storage data was recovered. Invalid records were removed; a backup was saved in this browser.';
...
export function load(): { data: StoredData; notice: string } {
  try {
    ...
    try {
      ...
      return { data: parsed.data, notice: 'Storage data was recovered, but a backup could not be saved.' };
    }
  } catch {
    return { data: emptyData(), notice: 'Storage is unavailable. Changes will last only while this page is open.' };
  }
}
