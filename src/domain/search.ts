...
function amountFilter(token: string): Filter | null {
  ...
  const amount = /^0+(?:[.,]0{1,2})?$/.test(match[2])
    ? { ok: true as const, minor: 0 }
    : parseAmount(match[2]);
  if (!amount.ok) return null;
  const boundary = amount.minor;
  switch (match[1]) {
    case '>':
      return (expense) => expense.amountMinor > boundary;
    case '<':
      return (expense) => expense.amountMinor < boundary;
    case '>=':
      return (expense) => expense.amountMinor >= boundary;
...
export function filterExpenses(expenses: Expense[], query: string, month: string): Expense[] {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token.startsWith('#')) {
      ...
      filter = dateFilter(
        `${prefix}-${range[1].padStart(2, '0')}`,
        `${prefix}-${range[2].padStart(2, '0')}`,
      );
      ...
      filter =
        note && !note.includes(':')
          ? (expense) => (expense.description ?? '').toLowerCase().includes(note)
          : null;
    }
    if (!filter) return [];
    filters.push(filter);
