...
export type CommandResult =
  ...
  | { ok: true; kind: 'repeat'; value: { description: string; amountMinor: number; category: Category; day: number } }
...
export function categoryFromTag(tag: string): Category | undefined {
  ...
  return CATEGORIES.find(category => category.toLowerCase() === (name === 'transport' ? 'transportation' : name));
}
...
function expenseDate(note: string, today: string, defaultDate: string): { date: string; description: string } {
  const suffix = note.match(/(?:^|\s)(today|yesterday|sun|mon|tue|wed|thu|fri|sat|\d{4}-\d{1,2}-\d{1,2}|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+\d{1,2})$/i);
...
export function parseCommand(text: string, today: string, defaultDate = today): CommandResult {
  ...
  if (command === '/budget') {
    ...
    if (parts[1].toLowerCase() === 'off') return { ok: true, kind: 'budget', category: category ?? null, amountMinor: null };
  ...
  if (command === '/repeat') {
    ...
    if (!amount.ok) return amount;
    return { ok: true, kind: 'repeat', value: {
      description: parts.slice(1, -2).join(' ').slice(0, 200), amountMinor: amount.minor,
      category: category ?? 'Other', day: Number(today.slice(-2)),
    } };
  }
  ...
  return { ok: true, kind: 'expense', value: {
    amountMinor: amount.minor, category: category ?? 'Other', date, description: description.slice(0, 200),
  } };
}
