...
it.each(['{broken', 'null', '[]', '4', '{}', '{"version":2,"expenses":[]}', '{"version":1,"expenses":{}}'])(
  'recovers malformed or unsupported data %s', raw => {
    expect(parseStoredData(raw)).toEqual({ data: empty, recovered: true });
  },
);
it.each([
  null, [], 3, {}, { ...expense, id: '' }, { ...expense, id: ' ' }, { ...expense, id: 2 },
  { ...expense, amountMinor: 0 }, { ...expense, amountMinor: -1 }, { ...expense, amountMinor: 1.5 },
  { ...expense, amountMinor: 100000000000 }, { ...expense, category: 'Bad' },
  { ...expense, date: 2026 }, { ...expense, date: '2026-02-30' },
  { ...expense, createdAt: -1 }, { ...expense, createdAt: null }, { ...expense, createdAt: 1.5 },
  { ...expense, description: 4 }, { ...expense, description: null },
...
it('drops duplicate ids', () => {
  expect(parseStoredData(JSON.stringify({ ...data, expenses: [expense, expense] }))).toEqual({ data, recovered: true });
});
