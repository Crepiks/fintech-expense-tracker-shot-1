...
it.each([
  ...
  openSelectedDay();
  submitEntry(command);
  expect(stored().expenses).toMatchObject([
    { date, amountMinor: 1200, category: 'Food', description: 'lunch' },
  ]);
...
it('creates and applies a recurring cost starting today from a different selected day', () => {
  ...
  expect(stored().settings.recurring).toMatchObject([
    { description: 'phone', day: 20, startDate: '2026-09-20', lastAppliedMonth: '2026-09' },
  ]);
  expect(stored().expenses).toMatchObject([
    { date: '2026-09-20', description: 'phone', amountMinor: 2500, fixed: true },
  ]);
});
