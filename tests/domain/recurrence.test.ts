...
const rule: RecurringCost = { id: 'rent', description: 'Rent', category: 'Housing', amountMinor: 80000, day: 31, startDate: '2026-01-01', lastAppliedMonth: null };
const state = (rules: RecurringCost[] = [rule]): StoredData => ({ ...empty, settings: { ...empty.settings, recurring: rules } });
...
it('materializes due months with stable ids and clamps short months', () => {
  ...
  expect(result.expenses).toEqual([
    { id: 'recurring:rent:2026-01', description: 'Rent', amountMinor: 80000, category: 'Housing', date: '2026-01-31', createdAt: 1769817600000, fixed: true, recurringId: 'rent' },
    { id: 'recurring:rent:2026-02', description: 'Rent', amountMinor: 80000, category: 'Housing', date: '2026-02-28', createdAt: 1772236800000, fixed: true, recurringId: 'rent' },
    { id: 'recurring:rent:2026-03', description: 'Rent', amountMinor: 80000, category: 'Housing', date: '2026-03-31', createdAt: 1774915200000, fixed: true, recurringId: 'rent' },
  ]);
...
it('includes the due date and leap day across year boundaries', () => {
  ...
  expect(applyRecurring(initial, '2024-02-29').expenses.map(item => item.date)).toEqual(['2023-12-31', '2024-01-31', '2024-02-29']);
});

it('skips an occurrence before the starting day and waits for future starts', () => {
  const initial = state([{ ...rule, day: 1, startDate: '2026-01-20' }, { ...rule, id: 'future', startDate: '2027-01-01' }]);
...
it('does not regenerate deleted prior occurrences', () => {
  ...
  expect(applyRecurring(initial, '2026-03-31').expenses.map(item => item.date)).toEqual(['2026-03-31']);
});
...
it('stops at capacity without dropping records or advancing unapplied months', () => {
  const initial = { ...state(), expenses: Array.from({ length: 9999 }, (_, i) => ({ ...expense, id: String(i) })) };
