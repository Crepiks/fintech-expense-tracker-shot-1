...
describe('calendar dates', () => {
  it.each(['2026-09-01', '2024-02-29', '2000-02-29', '0001-01-01', '9999-12-31'])(
    'accepts real date %s',
    (date) => expect(isValidDate(date)).toBe(true),
  );
  it.each([
    '',
    '2026-2-01',
...
it('normalizes a valid expense with an optional description', () => {
  expect(validateExpense({ ...draft, description: '  Lunch  ' }, '2026-09-19')).toEqual({
    ok: true,
    value: { amountMinor: 150000, category: 'Food', date: '2026-09-01', description: 'Lunch' },
    futureDate: false,
  });
});
...
it('normalizes an absent description to an empty string', () => {
  expect(validateExpense(draft, '2026-09-01')).toMatchObject({
    ok: true,
    value: { description: '' },
    futureDate: false,
  });
...
it('returns all invalid fields together', () => {
  expect(validateExpense({ amount: '', category: '', date: '' }, '2026-09-19')).toEqual({
    ok: false,
    errors: { amount: 'Enter an amount.', category: 'Choose a category.', date: 'Pick a date.' },
  });
});
it.each([
