...
it('has no budget metrics when the limit is cleared', () =>
  expect(computeBudget(null, 300000, '2026-09', '2026-09-19')).toBeNull());
it('floors safe daily spending in minor units and includes today', () => {
  expect(computeBudget(500000, 300000, '2026-09', '2026-09-19')).toEqual({
    remainingMinor: 200000,
    overBudget: false,
    progress: 60,
    pctUsed: 60,
    daysLeft: 12,
    safePerDayMinor: 16666,
  });
});
it('shows over-budget amounts and clamps the progress and safe allowance', () => {
  expect(computeBudget(50000, 60000, '2026-09', '2026-09-30')).toEqual({
...
it('handles exactly reaching the budget', () => {
  expect(computeBudget(50000, 50000, '2026-09', '2026-09-30')).toMatchObject({
    remainingMinor: 0,
    overBudget: false,
    safePerDayMinor: 0,
  });
...
it('counts leap-year February correctly', () => {
  expect(computeBudget(10000, 0, '2024-02', '2024-02-28')).toMatchObject({
    daysLeft: 2,
    safePerDayMinor: 5000,
    progress: 0,
  });
});
it.each(['2026-08', '2026-10'])('omits a daily allowance for non-current month %s', (month) => {
  expect(computeBudget(500000, 300000, month, '2026-09-19')).toEqual({
    remainingMinor: 200000,
    overBudget: false,
    progress: 60,
    pctUsed: 60,
...
it('counts every day in a 30-day month from its first day', () => {
  expect(computeBudget(30000, 0, '2026-04', '2026-04-01')).toMatchObject({
    daysLeft: 30,
