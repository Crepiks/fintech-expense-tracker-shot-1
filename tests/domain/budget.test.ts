...
it('floors safe daily spending in minor units and includes today', () => {
  expect(computeBudget(500000, 300000, '2026-09', '2026-09-19')).toEqual({ remainingMinor: 200000, overBudget: false, progress: 60, daysLeft: 12, safePerDayMinor: 16666 });
});
it('shows over-budget amounts and clamps the progress and safe allowance', () => {
  expect(computeBudget(50000, 60000, '2026-09', '2026-09-30')).toEqual({ remainingMinor: -10000, overBudget: true, progress: 100, daysLeft: 1, safePerDayMinor: 0 });
});
...
it.each(['2026-08', '2026-10'])('omits a daily allowance for non-current month %s', month => {
  expect(computeBudget(500000, 300000, month, '2026-09-19')).toEqual({ remainingMinor: 200000, overBudget: false, progress: 60, daysLeft: null, safePerDayMinor: null });
});
