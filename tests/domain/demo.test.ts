...
it('builds varied, valid expenses for the selected month and two previous months', () => {
  ...
  expect(new Set(result.data.expenses.map((item) => item.date.slice(0, 7)))).toEqual(
    new Set(['2026-07', '2026-08', '2026-09']),
  );
  expect(new Set(result.data.expenses.map((item) => item.category)).size).toBe(7);
  expect(result.data.expenses).toContainEqual(
  ...
  expect(parseStoredData(JSON.stringify(result.data))).toEqual({
    data: result.data,
    recovered: false,
  });
...
it('initializes example limits when optional settings collections are empty', () => {
  ...
  expect(prepareDemoData(state, '2026-09', '2026-09-19').data.settings.monthlyBudgetMinor).toBe(
    180000,
  );
});
...
it('does not duplicate records or overwrite edited demo records and settings on repeat', () => {
  ...
  const edited = {
    ...first,
    expenses: first.expenses.map((item, index) =>
      index === 0
        ? { ...item, amountMinor: 1234, description: 'My edited record', date: '2025-01-01' }
        : item,
    ),
...
it('deduplicates overlapping month ranges and adds only newly due purchases later', () => {
  ...
  expect(adjacent.data.expenses.filter((item) => item.date.startsWith('2026-09'))).toEqual(
    later.data.expenses.filter((item) => item.date.startsWith('2026-09')),
  );
  expect(
    adjacent.data.expenses
      .filter((item) => item.date.startsWith('2026-10'))
      .every((item) => item.fixed),
  ).toBe(true);
  expect(new Set(adjacent.data.expenses.map((item) => item.id)).size).toBe(
    adjacent.data.expenses.length,
  );
});

it.each(['2024-02', '2025-02', '2026-01', '0001-01', '9999-12'])(
  'handles short months, year rollover and supported date bounds: %s',
  (month) => {
...
it('rejects the whole batch above the record limit without modifying data or settings', () => {
  const state = {
    ...empty,
    expenses: Array.from({ length: 9999 }, (_, index) => ({ ...expense, id: String(index) })),
  };
  const result = prepareDemoData(state, '2026-09', '2026-09-19');
...
it('allows a batch exactly at capacity and repeated loads at capacity', () => {
  ...
  const state = {
    ...empty,
    expenses: Array.from({ length: 10000 - batch.added }, (_, index) => ({
      ...expense,
      id: String(index),
    })),
  };
  ...
  expect(result.error).toBe('');
  expect(prepareDemoData(result.data, '2026-09', '2026-09-19')).toEqual({
    data: result.data,
    added: 0,
    error: '',
  });
