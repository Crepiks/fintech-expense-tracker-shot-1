...
import { appTestLifecycle, csvFile, navigate, openEntry, receiveRemote, seed, stored } from './appHelpers';
...
const selectCsv = () => fireEvent.change(screen.getByLabelText('Choose CSV'), { target: { files: [csvFile(csv)] } });
...
it('imports parsed expenses with real generated IDs and persists them after reload', async () => {
  ...
  expect(stored().expenses[0]).toEqual({
    id: '00000000-0000-4000-8000-000000000001', amountMinor: 1250, category: 'Food',
    description: 'Lunch', date: '2026-09-19', createdAt: new Date(2026, 8, 19, 12).getTime(),
  });
  unmount();
  render(<App />);
  navigate('ledger');
...
it('lists later imported rows first for a shared date and recent-entry suggestions', async () => {
  ...
  expect(screen.getAllByRole('button', { name: /^Edit / }).map(button => button.getAttribute('aria-label'))).toEqual([
    'Edit Taxi', 'Edit Dinner', 'Edit Lunch', 'Edit Taxi', 'Edit Dinner', 'Edit Lunch',
  ]);
  openEntry();
  const suggestions = within(screen.getByRole('dialog', { name: 'New entry' })).getAllByRole('button', { name: /^↺/ });
...
it('cancels an import if another tab fills the ledger after preview', async () => {
  const records = Array.from({ length: 9999 }, (_, index) => ({ ...expense, id: String(index), date: '2026-08-01' }));
  seed({ ...empty, expenses: records });
  render(<App />);
  openImport();
  selectCsv();
  await screen.findByText('Ready to import 1 expenses.');
  receiveRemote({ ...empty, expenses: [...records, { ...expense, id: 'remote', date: '2026-08-01' }] });
  fireEvent.click(screen.getByRole('button', { name: 'Import expenses' }));
  expect(screen.getByText('Import cancelled: the 10,000-record limit was reached.')).toBeInTheDocument();
...
it('protects an import from the entry shortcut and cancels without changing data', () => {
  ...
  fireEvent.click(within(screen.getByRole('dialog', { name: 'Import CSV' })).getByRole('button', { name: 'cancel' }));
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
