...
const select = (selected: File) => fireEvent.change(screen.getByLabelText('Choose CSV'), { target: { files: [selected] } });
...
it.each([[], null])('clears a previous preview when the file selection is cancelled (%j)', async files => {
...
it('rejects a batch that would exceed the total ledger capacity', async () => {
  ...
  select(file());
  expect(await screen.findByRole('alert')).toHaveTextContent('This import would exceed the 10,000-expense limit.');
  expect(screen.getByRole('button', { name: 'Import expenses' })).toBeDisabled();
  expect(callbacks.onImport).not.toHaveBeenCalled();
});
...
it('explains file read failures and allows a subsequent valid file', async () => {
  ...
  expect(await screen.findByRole('alert')).toHaveTextContent('Could not read this file. Try selecting it again.');
  expect(screen.getByLabelText('Choose CSV')).toBeEnabled();
  select(file());
  await screen.findByText('Ready to import 2 expenses.');
  expect(screen.queryByRole('alert')).not.toBeInTheDocument();
