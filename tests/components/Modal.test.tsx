...
it('opens an accessible dialog containing its supplied content', () => {
  render(<Modal title="Edit expense" onClose={vi.fn()}><p>Expense details</p></Modal>);
...
it('preserves a supplied modal class', () => {
  render(<Modal title="Import expenses" className="import-dialog" onClose={vi.fn()}><p>Choose a file</p></Modal>);
  expect(screen.getByRole('dialog', { name: 'Import expenses' })).toHaveClass('modal', 'import-dialog');
});

it('uses the native Escape cancel event to request controlled dismissal', () => {
  const onClose = vi.fn();
  render(<Modal title="Add expense" onClose={onClose}><button>Save</button></Modal>);
...
it('closes and restores the opener focus when the parent dismisses the dialog', () => {
  function Example() {
    ...
    return <><button onClick={() => setOpen(true)}>Open entry</button>{open && <Modal title="Entry" onClose={() => setOpen(false)}><button>Amount</button></Modal>}</>;
  }
  render(<Example />);
