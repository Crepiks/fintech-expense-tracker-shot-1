...
it('opens a real validation scenario with ten passing rows and closes', () => {
  ...
  expect(screen.getByRole('dialog', { name: 'Validation scenario (section 6 of the brief)' })).toBeInTheDocument();
...
it('clearly reports a failed check instead of always claiming success', () => {
  render(<ScenarioDialog results={[{ step: 'After adding', label: 'Total', expected: '3,000', actual: '2,000', pass: false }]} onClose={vi.fn()} onRunAgain={vi.fn()} />);
