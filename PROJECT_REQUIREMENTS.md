...
Algorithm: 1. s = input.trim(), remove all whitespace incl. \u00A0 2. empty -> "Enter an amount." 3. starts with "-" -> "Amount must be greater than 0." 4. if s matches ^\d{1,3}(,\d{3})+(\.\d+)?$
         it is thousands-grouped: strip the commas
       else replace a single "," with "."
    5. not ^\d+(\.\d+)?$ -> "Use digits only, e.g. 1500 or
1500.50." 6. more than 2 decimals -> "Use at most two decimal places." 7. [int, frac=''] = s.split('.')
minor = Number(int) * 100 + Number(frac.padEnd(2,'0'))
(string padding, never parseFloat(x) * 100) 8. minor === 0 -> "Amount must be greater than 0." 9. minor > MAX -> "Amount is too large."
...
load(): StoredData - missing key -> empty default - try JSON.parse, check version === 1, expenses is an array,
each item has valid id / integer amountMinor > 0 /
known category / valid date. Drop invalid items. - on parse failure: copy raw string to key + ':backup',
